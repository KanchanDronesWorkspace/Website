...
import { blogFormSchema, validateWithZod } from '@/lib/schemas/validation'
...
function BlogEditor() {
  ...
  const handleAutoSave = async () => {
    if (!formData.title || !formData.content || saving) return
  ...
  const handleSubmit = async (status: 'draft' | 'submit') => {
    const validation = validateWithZod(blogFormSchema, formData)
    ...
    try {
      ...
      if (isEditing && blogId) {
        await BlogService.updateBlog(blogId, formData, user.id)
        toast.success('Blog updated!', {
          description: 'Your blog has been successfully updated.'
        })
      } else {
        await BlogService.createBlog(formData, user.id)
      ...
      <section className="max-w-7xl mx-auto px-5 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-6">
                ...
                {formData.cover_image_url && (
                  <div className="relative group">
                    ...
                    <div className="absolute top-3 right-3 bg-black/70 backdrop-blur-sm text-white px-3 py-1.5 rounded-lg text-xs font-medium flex items-center space-x-1">
                      {ImageUploadService.isValidImageUrl(formData.cover_image_url) ? (
                        <>
                          <span>Uploaded</span>
                        </>
                      ) : (
            ...
            <div className="bg-card/50 backdrop-blur-sm p-6 rounded-xl border border-border/50">
              <label htmlFor="tags" className="block text-sm font-medium text-foreground mb-3">
                Tags
              </label>
            ...
            <div className="flex justify-between space-x-4 pt-4">
              <button
                onClick={() => router.back()}
                className="px-6 py-3 border border-border/50 text-foreground rounded-lg hover:bg-muted/50 transition-all duration-200 font-medium"
              >
                Cancel
              </button>
              <div className="flex space-x-4">
                <button
                  ...
                  disabled={loading}
                  className="px-6 py-3 bg-secondary text-foreground rounded-lg hover:bg-secondary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium"
                >
...
export default withAuth(BlogEditor, 'employee')
